...
router.delete('/:id', async (req, res) => {
    try {
        const { id } =  req.params
        const db = await connect();
        const result = await db.collection('categories').findOneAndDelete({ _id: ObjectID(id) })
        res.json({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const db = await connect();
        const result = await db.collection('categories').updateOne({ _id: ObjectID(id) }, { $set: updateCategory })
        res.status(200).json({
